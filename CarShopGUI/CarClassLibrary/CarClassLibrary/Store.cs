...
namespace CarClassLibrary
{
    public class Store
    {
        ...
        private String[] Honda = {"Accord", "Odyssey", "Civic  ", "Pilot   ", "CR-V", "Fit     ", "S2000" };
        ...
        //I got lazy and just added spaces to the names to make the display tab it evenly.
        ...
        public List<Car> generatedCars(int amount)
        {
            ...
            for (int i = 0; i < amount; i++)
            {
                ...
                bool rNew = false;
                bool rElectric = false;
                int rYear = 0;
                int rRange = random.Next(50, 500);
                int check = random.Next(1, 100);
                if (check%2 == 0)
                {
                    rNew = true;
                    rYear = random.Next(2019,2021);
                    // makes sure its new, both the year and also status

                }else
                {
                    rYear = random.Next(1990, 2018);
                }
